docReady(async () => {
	...
	// API Request (Dummy)
	let callAPI = false;
	document.getElementById('callResponseAPI').addEventListener('click', async () => {
		if (!callAPI) {
			callAPI = true;
			...
			if (responseFromAPI.status > 199 && responseFromAPI.status < 300) {
				document.getElementById('responseAPI').innerHTML = JSON.stringify(responseFromAPI.response);
			} else {
				callAPI = false;
			}
		}
	...
	// Random String
	document.getElementById('callGenerateRandomString').addEventListener('click', () => {
		document.getElementById('generateRandomString').innerHTML = randomString(20);
	});
});